...
src: $(OBJECTS)
	...
	$(CXX) -mcpu=cortex-m3 -mthumb -Os -Wl,--gc-sections -T..\arduino\hardware\sam\1.6.12\variants\arduino_due_x\linker_scripts\gcc\flash.ld "-Wl,-Map,$(OUTPUT)\mapping.map" -o $(OUTPUT)\$(EXEC).elf -L$(OUTPUT) -Wl,--cref -Wl,--check-sections -Wl,--gc-sections -Wl,--entry=Reset_Handler -Wl,--unresolved-symbols=report-all -Wl,--warn-common -Wl,--warn-section-align -Wl,--start-group -u _sbrk -u link -u _close -u _fstat -u _isatty -u _lseek -u _read -u _write -u _exit -u kill -u _getpid $(ARDUINO_DUE_AR) $(ADAFRUIT_NEOPIXEL_AR) $(OUTPUT)\$(EXEC).a -Wl,--end-group -lm -lgcc
	$(OBJC) -O binary $(OUTPUT)\$(EXEC).elf $(OUTPUT)\$(EXEC).bin
	$(SIZE) -A $(OUTPUT)\$(EXEC).elf

%.o : %.cpp $(OUTPUT)
	@echo ("Compiling source files . . .);
	$(CXX) $(CXXFLAGS) $(ADAFRUIT_NEOPIXEL_I) $< -o $(OUTPUT)\$@
